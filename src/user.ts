...
export default class User extends EventEmitter {
    ...
    private _connection:websocket.connection|null = null;
    ...
    private _incomes:Incomes;
    private _upkeeps:Upkeeps;
    ...
    public set connection( c:websocket.connection ) {        
        this._connection = c;
        this._connection.on( 'message', msg => this.onMessage( msg ) );
    }

    private onMessage( msg:websocket.IMessage ):void {
	...
	async load() {
        ...
        let data = await dbase.getOne( "SELECT username, email, current_round, avatar, gems, sex, users_bots.type AS bot, banned, banned_reason, banned_until FROM users LEFT JOIN users_bots ON users_bots.userid = users.id WHERE users.id = " + this._id + " LIMIT 1" );
		...
		let data = await dbase.getOne( "SELECT username, email, current_round, avatar, gems, sex, users_bots.type AS bot, banned, banned_reason, banned_until FROM users LEFT JOIN users_bots ON users_bots.userid = users.id WHERE users.id = " + this.id + " LIMIT 1" );
		if( !data ) {
		...
		if( this.currentRound ) {
			data = await dbase.getOne( "SELECT * FROM users_rounds WHERE userid = " + this.id + " AND roundid = " + this.currentRound );			
			if( data ) {
				console.log( "-----------------------" );
				console.log( data );
				console.log( "-----------------------" );
				...
				data.user.buildings = {};				
				const buildings = await dbase.get( "SELECT quantity, type FROM users_rounds_buildings INNER JOIN buildings ON buildingid = buildings.id WHERE userid = " + this.id + " AND roundid = " + this.currentRound + " ORDER BY quantity DESC" );
				...
				data.user.units = {};
				const units = await dbase.get( "SELECT quantity, type FROM users_rounds_units INNER JOIN units ON units.id = unitid WHERE userid = " + this.id + " AND roundid = " + this.currentRound );
				console.log( "==============================" );
				...
				let events = await dbase.getOne( "SELECT COUNT(id) AS count FROM events WHERE userid = " + this.id + " AND roundid = " + this.currentRound + " AND unread = 1 AND deleted = 0" );
				data.user.events = events ? events.count : 0;
				
				let mails = await dbase.getOne( "SELECT COUNT(id) AS count FROM mails WHERE recipient = " + this.id + " AND unread = 1 AND recipientview = 1" );
    ...
    async loadRoundData() {
        ...
        let result:RowDataPacket = await dbase.getOne( queries.info, [ this._id, this._round ] );
        if( !result ) throw "AHHHH";

        console.log( result );

        //this._power = result.power;
        
        //this._resources.gold = 
	...
	async update( expired ) {
		...
		/*try {
			if( this.currentRound ) {
				...
				let data = await dbase.getOne( "SELECT * FROM users_rounds WHERE userid = " + this.id + " AND roundid = " + this.currentRound );
				if( data ) {
					this.gold = parseFloat( data.gold );
					this.land = parseFloat( data.land );
					this.landFree = parseFloat( data.land_free );
					...
					const events = await dbase.getOne( eventQuery );
	...
	async updateDeltas( user ) {
		...
		let data = await dbase.getOne( userQuery );
		if( data ) {
			...
			const buildings = await dbase.get( buildingQuery )
			if( buildings ) {
				for( var b in buildings ) {
			...
			const units = await dbase.get( unitQuery );
			if( units ) {
				for( var u in units ) {
	...
	/*async login( $username, $password, $token, $os ) {
		...
		const data = await dbase.getOne( "SELECT id, password, username FROM users WHERE BINARY username = '" + $username + "' LIMIT 1" );
		...
		if( compare ) {
			...
			const ban = await dbase.get( banQuery );
			if( ban && ban.length >= 1 ) {
			...
			let tokenResult = await dbase.getOne( tokenQuery );
			if( !tokenResult ) {				
				let insertResult = await dbase.execute( "INSERT INTO users_push_tokens SET userid = " + this.id + ", os = '" + $os + "', token = '" + $token + "', date = UNIX_TIMESTAMP()" );								
	...
	async changeEmail( $password, $email ) {
		...
		const check = await dbase.getOne( userQuery );
	...
	async changePassword( $oldPassword, $newPassword ) {
		...
		let result = await dbase.getOne( userQuery );		
	...
	async checkForBannedDupes() {
		...
		const result = await dbase.getOne( query );
		if( result ) {
	...
	async checkForTokenDupes( $token ) {
		...
		const results = await dbase.get( query );
	...
	async recordDupe( $p1, $p2, $type, $connection ) {
		...
		result = await dbase.getOne( query );
	...
	async banUser( $id, $reason, $length ) {
		...
		let result = await dbase.execute( query );
		...
		result = await dbase.execute( query );
		if( result.affectedRows === 0 ) {		
	...
	async joinRound( $rid ) {
		...
		var round1 = await dbase.getOne( "SELECT current_round FROM users WHERE id = " + this.id );
		...
		let rankResult = await dbase.getOne( rankQuery );
		if( rankResult ) {
			...
			rankResult = await dbase.getOne( rankQuery );
			if( rankResult ) {				
	...
	async playRound( $rid ) {
		...
		if( round ) {
			if( round.active ) {
				const check = await dbase.getOne( checkQuery );
				if( check ) {
					const connection = await dbase.beginTransaction();
					const result = await connection.query( updateQuery );
					if( result && result[ 0 ].affectedRows == 1 ) {
	...
	async getSummary() {
		...
		let results = await dbase.getOne( eventQuery );
		...
		results = await dbase.getOne( snapshotQuery );
	...
	async destroyBuildings( $type, $quantity ) {
		...
		const building = await dbase.getOne( buildingQuery );
		if( building ) {				
	...
	async fireUnits( $type, $quantity ) {
		...
		if( unit ) {
			...
			const units = await dbase.get( query );
			if( units ) {
				let data = {};
	...
	async calculatePower( $userid, $roundid ) {		
		...
		const army = await dbase.get( armyQuery );
		...
		const landResult = await dbase.getOne( landQuery );		
		if( landResult ) {
	...
	async claimDaily() {
		...
		let data = await dbase.getOne( query );
		if( data ) {
			if( !data.claimed ) {
				data.level += 1;
	...
	async getItems() {
		...
		const items = await dbase.get( query );
		if( items ) {
			console.log( items );
	...
	async useItem( $item ) {
		...
		if( item.onUse ) {
			...
			const items = await dbase.get( itemsQuery );
	...
	async getAvatars() {
		...
		const avatars = await dbase.get( query );
		if( avatars ) {
	...
	async getEvents( $page, $per ) {
		...
		let data = {};
		const result = await dbase.getOne( "SELECT COUNT(id) AS total FROM events WHERE userid = " + this.id + " AND roundid = " + this.currentRound + " AND deleted = 0" );
		...
		const events = await dbase.get( eventQuery );
		...
		if( events && events.length > 0 ) {
			...
			if( !result || results[ 0 ].affectedRows < 1 ) await dbase.rollback( connection );
			else await dbase.commit( connection );					
		}
		
		data = {};
		const count = await dbase.getOne( "SELECT COUNT(id) AS count FROM events WHERE userid = " + this.id + " AND roundid = " + this.currentRound + " AND unread = 1 AND deleted = 0" );		
	...
	async deleteAllEvents() {
		...
		if( !result || result[ 0 ].affectedRows < 1 ) await dbase.rollback( connection );
		else await dbase.commit( connection );
	...
	async buyenergy() {
		...
		const user = await dbase.getOne( userQuery );
		if( !user ) {
	...
	async getMarketItemInfo( $item ) {
		...
		const item = await dbase.getOne( query );
		if( item ) {
			var packet = {};
	...
	async buyMarket( $type, $item, $quantity, $price ) {
		...
		const item = await dbase.getOne( marketQuery );
		if( item ) {
			...
			if( data.price <= $price ) this.buyResource( $item, $quantity, $price );
				...
				const history = await dbase.getOne( historyQuery );
				if( history ) {
	...
	async sendMail( $recipient, $message ) {
		...
		const user = await dbase.getOne( idQuery );
		if( !user ) {
		...
		let blocked = await dbase.getOne( blockQuery );
		...
		blocked = await dbase.getOne( blockQuery );
	...
	async sendShout( $shout ) {
		...
		const check = await dbase.getOne( "SELECT * FROM shoutbox WHERE userid = " + this.id + " AND time > UNIX_TIMESTAMP() - 60" );
	...
	async getContacts() {		
		...
		packet.friends = await dbase.get( "SELECT avatar, username FROM users INNER JOIN contacts ON contactid = users.id WHERE contacts.userid = " + this.id + " AND type='friend'" );
		packet.enemies = await dbase.get( "SELECT avatar, username FROM users INNER JOIN contacts ON contactid = users.id WHERE contacts.userid = " + this.id + " AND type='enemy'" );
		packet.blocked = await dbase.get( "SELECT avatar, username FROM users INNER JOIN contacts ON contactid = users.id WHERE contacts.userid = " + this.id + " AND type='blocked'" );
	...
	async getConversation( $username, $page ) {
		...
		const recipient = await dbase.getOne( userQuery );
		if( recipient ) {
			...
			const mails = await dbase.get( retrieveQuery );
			this.dispatch( "MAIL_DETAILS_RETRIEVED", mails );
			
			const connection = await dbase.beginTransaction();
			const result = await connection.query( readQuery );
			if( result && result[ 0 ].affectedRows >= 1 ) await dbase.commit( connection );			
	...
	async getMails( $page, $per ) {
		...
		const result = await dbase.getOne( countQuery );		
		if( result ) {
			...
			data.mails = await dbase.get( retrieveQuery );			
	...
	async deleteMail( $name ) {
		...
		const user = await dbase.getOne( userQuer );
		if( user ) {
	...
	async removeFriend( $name ) {
		...
		let friend = await dbase.getOne( userQuery );
		if( !friend ) {
	...
	async addEnemy( $name ) {
		...
		const userQuery = "SELECT id FROM users WHERE username = '" + $name + "'";
		let enemy = await dbase.getOne( userQuery );
		if( !enemy ) {
		...
		let result = await dbase.getOne( enemyQuery );
		if( result ) {
	...
	async addBlocked( $name ) {
		...
		const userQuery = "SELECT id FROM users WHERE username = '" + $name + "'";
		let block = await dbase.getOne( userQuery );
		if( !block ) {
		...
		let result = await dbase.getOne( blockQuery );
		if( result ) {
	...
	async getNearbyRankings( $per ) {
		...
		let rank = await dbase.getOne( rankQuery );
		if( rank ) {
			rank = rank.rank;
			...
			let ranks = await dbase.get( rankQuery );
	...
	async checkJobs() {
		...
		let data = await dbase.getOne( "SELECT COUNT(id) AS total FROM users_jobs WHERE userid = " + this.id + " AND ( claimed = 0 OR claimed > UNIX_TIMESTAMP() - " + this.jobThrottleTime + " )" );
	...
	async createJob() {
		...
		let job = guid.v4();
		let result = await dbase.execute( "INSERT INTO users_jobs SET guid = '" + job + "', userid = " + this.id + ", reward = " + reward.id + ", claimed = 0" );		
	}
	
	async clearJobs() {
		this.debug( "clearJobs" );		
		await dbase.execute( "DELETE FROM users_jobs WHERE userid = " + this.id );
	}
	...
	async getJobs() {
		...
		let jobs = await dbase.get( "SELECT guid, reward FROM users_jobs WHERE userid = " + this.id + " AND claimed = 0" );
		for( var job in jobs ) {			
	...
	async getTargets() {
		...
		const data = await dbase.getOne( powerQuery );
		if( !data ) {
		...
		const targets = await dbase.get( targetQuery );
	...
	async getFights( $page, $per ) {
		...
		let info = await dbase.getOne( countQuery );
		if( !info ) {
		...
		data.fights = await dbase.get( retrieveQuery );
	...
	*//*
	...
	async processCombatDefeat( $loser ) {
		...
		const data = await dbase.getOne( "SELECT land FROM users_rounds WHERE userid = " + $loser.id + " AND roundid = " + this.currentRound );
		...
		if( gain > 0 ) {
			...
			let buildings = await dbase.get( buildingQuery );
	...
	*/
	...
	async attack( $target ) {
		...
		let c = new Combat( dbase, this.id, $target, this.currentRound );
		await c.Load();
		await c.Simulate();
		...
		let attackingArmy = await dbase.get( "SELECT unitid, quantity FROM users_rounds_units WHERE userid = " + this.id + " AND roundid = " + this.currentRound );		
		if( !attackingArmy || attackingArmy.length < 1 ) return this.dispatchError( "You have no army to attack with" );
		let defendingArmy = await dbase.get( "SELECT unitid, quantity FROM users_rounds_units WHERE userid = " + defender.id + " AND roundid = " + this.currentRound );			
		...
		try {
			...
			if( attackerLosses.error ) { await dbase.rollback( connection ); logger.logError( "Error Attacking: " + attackerLosses.error ); return this.dispatchError( "Error attacking" ); }
			
			let defenderLosses = await this.processUnitLosses( defender, connection );
			if( defenderLosses.error ) { await dbase.rollback( connection ); logger.logError( "Error Attacking: " + defenderLosses.error ); return this.dispatchError( "Error attacking" ); }					
			...
			if( combat.victory ) {
				...
				let totalBuildings = await dbase.getOne( "SELECT SUM(quantity) AS total FROM users_rounds_buildings WHERE userid = " + defender.id + " AND roundid = " + this.currentRound );				
	...
	async raid( $target ) {
		...
		let attackingArmy = await dbase.get( "SELECT unitid, quantity, quantity * ( attack + defense ) / ( 1 + ranged ) AS totalpower FROM users_rounds_units INNER JOIN units ON units.id = unitid WHERE userid = " + this.id + " AND roundid = " + this.currentRound + " ORDER BY totalpower DESC LIMIT 1" );
		if( !attackingArmy || attackingArmy.length < 1 ) return this.dispatchError( "You have no army to attack with" );
		let defendingArmy = await dbase.get( "SELECT unitid, quantity, quantity * ( attack + defense ) / ( 1 + ranged ) AS totalpower FROM users_rounds_units INNER JOIN units ON units.id = unitid WHERE userid = " + defender.id + " AND roundid = " + this.currentRound + " ORDER BY totalpower DESC" );
	...
	*//*
	...
	async constrainUnits( $id ) {
		...
		if( result && result[ 0 ].affectedRows == 1 ) await dbase.commit( connection );
		else await dbase.rollback( connection );
	}
	...
	async getFight( $fight ) {
		...
		const fight = await dbase.getOne( query );
	...
	async deleteFight( $fight ) {
		...
		const fight = await dbase.getOne( fightQuery );
		if( !fight ) {
	...
	async explore( $energy ) {
		...
		let land = await dbase.getOne( "SELECT land FROM users_rounds WHERE userid = " + this.id + " AND roundid = " + this.currentRound );
		if( !land ) {
	...
	async build( $type, $quantity, $restarted ) {		
		...
		let userData = await dbase.getOne( userQuery );		
		if( !userData ) {
		...
		let buildingData = await dbase.getOne( buildingQuery );		
		...
		let data = {};
		const buildings = await dbase.get( usersBuildingsQuery );						
	...
	async recruit( $type, $quantity, $restarted ) {		
		...
		var data = {};
		const units = await dbase.get( unitsQuery );
	...
	async getNotificationSettings() {
		...
		const result = await dbase.get( query );
		console.log( result );
	...
	async setNotificationSetting( $type, $value ) {
		...
		if( query ) {			
			const result = await dbase.execute( query );
