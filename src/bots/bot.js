...
class Bot extends User {
	...
	async findRounds() {
		...
		this.rounds = await dbase.get( "SELECT id FROM rounds WHERE active = 1" );		
	...
	async process() {
		...
		let result = await dbase.getOne( "SELECT id FROM users_rounds WHERE userid = " + this.id + " AND roundid = " + this.currentRound );
		if( result ) {
	...
	async load() {
		...
		let data = await dbase.getOne( "SELECT username, email, current_round, avatar, gems, sex, users_bots.type AS bot, users_bots.params AS params FROM users LEFT JOIN users_bots ON users_bots.userid = users.id WHERE users.id = " + this.id + " LIMIT 1" );		
		...
		if( this.currentRound ) {
			...
			this.buildings = await dbase.get( "SELECT quantity, type FROM users_rounds_buildings INNER JOIN buildings ON buildingid = buildings.id WHERE userid = " + this.id + " AND roundid = " + this.currentRound + " ORDER BY quantity DESC" );
			this.units = await dbase.get( "SELECT quantity, type FROM users_rounds_units INNER JOIN units ON units.id = unitid WHERE userid = " + this.id + " AND roundid = " + this.currentRound );					
		}
	}
