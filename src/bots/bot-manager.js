...
class BotManager {
	...
	async clearBotData() {
		...
		dbase.commit( connection );			
	}

	async spawnBots( clear ) {
		this.debug( "SpawnBots" );

		const bots = await dbase.get( "SELECT userid, type, active FROM users_bots WHERE userid >= 4 AND userid <= 34" );
		for( var i = 0; i < bots.length; i++ ) {
			...
			switch( bot.type ) {
				case "explorer": this.bots.push( new ExploreBot( bot.userid, dbase ) ); break;
				case "builder":	this.bots.push( new BuilderBot( bot.userid, dbase ) ); break;
				case "market": this.bots.push( new MarketBot( bot.userid, dbase ) ); break;
				case "recruiter": this.bots.push( new RecruiterBot( bot.userid, dbase ) ); break;
				case "war": this.bots.push( new WarBot( bot.userid, dbase ) ); break;
				case "behavior": this.bots.push( new BehaviorBot( bot.userid, dbase ) ); break;
