...
var HTTP = require( './admin' );

var cron = require( 'node-schedule' );

var http = require('http').Server( HTTP.app );
var io = require('socket.io')( http );

var User = require( './user' );
var CronManager = require( './crons' );
...
var JobRewardManager = require( './job-reward-manager' );
...
//var bot = new Bot( 5, database );
//bot = new Bot( 6, database );
//var bot = new Bot( 7, database );
...
NotificationManager.Database = database;
...
io.on( 'connection', function( socket ) {	
	...
	socket.on( "VALIDATE_CLIENT", async function( data ) {
		var version = parseInt( Buffer.from( data.version, "base64" ).toString(), 10 );
		var platform = Buffer.from( data.platform, "base64" ).toString();				
		var query = "SELECT minimum_version FROM apps WHERE platform = '" + platform + "' LIMIT 1";			
	...
	socket.on( "EXPLORE", async function( data ) {
		var energy = parseInt( Buffer.from( data.energy, "base64" ).toString(), 10 );
	...
	socket.on( "BUILD", function( data ) {
		var quantity = parseInt( Buffer.from( data.quantity, "base64" ).toString(), 10 );
		var type = Buffer.from( data.type, "base64" ).toString();
	...
	socket.on( "SEND_SHOUT", function( data ) {
		var shout = Buffer.from( data.shout, "base64" ).toString();
	...
	socket.on( "CHANGE_PASSWORD", function( data ) {
		if( this.user ) {
			var oldPassword = Buffer.from( data.oldPassword, "base64" ).toString();	
			var newPassword = Buffer.from( data.newPassword, "base64" ).toString();
	...
	socket.on( "CHANGE_EMAIL", function( data ) {
		if( this.user ) {
			var password = Buffer.from( data.password, "base64" ).toString();	
			var email = Buffer.from( data.email, "base64" ).toString();
	...
	socket.on( "GET_MARKET_ITEM_INFO", function( data ) {
		if( this.user ) {
			var item = Buffer.from( data.item, "base64" ).toString();		
	...
	socket.on( "SET_AVATAR", function( data ) {
		if( this.user ) {
			var avatar = parseInt( Buffer.from( data.avatar, "base64" ).toString(), 10 );
	...
	socket.on( "GET_FIGHTS", function( data ) {
		if( this.user ) {
			var page = parseInt( Buffer.from( data.page, "base64" ).toString(), 10 );
			var per = parseInt( Buffer.from( data.per, "base64" ).toString(), 10 );
	...
	socket.on( "GET_FIGHT", function( data ) {
		if( this.user ) {
			var guid = Buffer.from( data.fight, "base64" ).toString();
	...
	socket.on( "ATTACK", function( data ) {
		if( this.user ) {
			var target = Buffer.from( data.target, "base64" ).toString();
	...
	socket.on( "CLAIM_JOB", function( data ) {
		if( this.user ) {
			var job = Buffer.from( data.job, "base64" ).toString();
	...
	socket.on( "ADD_FRIEND", function( data ) {
		if( this.user ) {
			var name = Buffer.from( data.name, "base64" ).toString();
	...
	socket.on( "SUBMIT_CONTACT", function( data ) {		
		if( this.user ) {
			var message = Buffer.from( data.message, "base64" ).toString();
	...
	socket.on( "REGISTER_TOKEN", async function( data ) {
		...
		// PRODUCTION ALERT
		// TO DO : Remove test_type when going Live
		/*pushClient.addDevice( { device_type: os === "ios" ? 0 : 1, identifier:socket.token, test_type:1 }, function (err, httpResponse, data) {      
			socket.oneSignalID = data.id;
			...
			/*if( !err ) {
				...
				pushClient.sendNotification(firstNotification, function (err, httpResponse,data) {      
					if (err) {      
	...
	socket.on( "SEARCH_USERS", async function( data ) {
		if( this.user ) {
			var search = Buffer.from( data.search, "base64" ).toString();								
			var query = "SELECT username, avatar FROM users WHERE username LIKE '%" + search + "%' ORDER BY ( CASE WHEN username = '" + search + "' then 1 ELSE 0 END) + ( CASE WHEN username LIKE '" + search + "%' THEN 1 ELSE 0 END ) + ( CASE WHEN username LIKE '%" + search + "%' THEN 1 ELSE 0 END ) DESC;"
	...
	socket.on( "GET_USER", function( data ) {
		if( this.user ) {
			var username = validator.escape( Buffer.from( data.name, "base64" ).toString() );
	...
	socket.on( "DELETE_EVENT", function( data ) {
		if( this.user ) {
			var id = parseInt( Buffer.from( data.id, "base64").toString(), 10 );
	...
	socket.on( "SEND_MAIL", function( data ) {
		if( this.user ) {
			var recipient = Buffer.from( data.username, "base64" ).toString();
	...
	socket.on( "GET_MAILS", function( data ) {
		if( this.user ) {			
			var page = parseInt( Buffer.from( data.page != null ? data.page : 1, "base64" ).toString(), 10 );
			var per = parseInt( Buffer.from( data.per != null ? data.per : 15, "base64" ).toString(), 10 );
	...
	socket.on( "GET_MAIL_DETAILS", function( data ) {
		if( this.user ) {
			var page = parseInt( Buffer.from( data.page, "base64" ).toString(), 10 );
			var username = Buffer.from( data.username, "base64" ).toString();
	...
	socket.on( "RECOVER_PASSWORD", async function( data ) {
		var email = validator.escape( Buffer.from( data.email, "base64" ).toString() );
	...
	socket.on( "SEND_ITEMS", function( data ) {
		if( this.user ) {
			var to = Buffer.from( data.to, "base64").toString();
			var category = Buffer.from( data.category, "base64").toString();
			var type = Buffer.from( data.type, "base64").toString();
			var quantity = parseInt( Buffer.from( data.quantity, "base64").toString(), 10 );
	...
	socket.on( "JOIN_ROUND", function( data ) {
		if( this.user ) {
			var round = parseInt( Buffer.from( data.round, "base64").toString(), 10 );
	...
	socket.on( "USE_ITEM", function( data ) {
		if( this.user ) {
			var item = parseInt( Buffer.from( data.item, "base64" ).toString(), 10 );
	...
	socket.on( "MARKET_BUY", function( data ) {
		if( this.user ) {
			...
			var item = Buffer.from( data.item, "base64" ).toString();
			var quantity = parseInt( Buffer.from( data.quantity, "base64" ).toString(), 10 );
			var price = parseFloat( Buffer.from( data.price, "base64" ).toString(), 10 );
	...
	socket.on( "MARKET_POST", function( data ) {
		if( this.user ) {			
			...
			var item = Buffer.from( data.item, "base64" ).toString();
			var quantity = parseInt( Buffer.from( data.quantity, "base64" ).toString(), 10 );
			var price = parseInt( Buffer.from( data.price, "base64" ).toString(), 10 );
